...
function call(command,data={}){
  if(!desktop)return Promise.reject(new Error('桌面服务尚未连接，请通过课页启动程序打开。'));
  return new Promise((resolve,reject)=>{const id=String(++sequence);pending.set(id,{resolve,reject});desktop.request(id,command,JSON.stringify(data));});
}
...
function cover(m){
  ...
  const title=Array.from(m.title),line1=title.slice(0,15).join(''),line2=title.slice(15,29).join('')+(title.length>29?'…':'');
  return `<svg class="slide" viewBox="0 0 640 360" role="img" aria-label="${esc(m.title)} 自动生成封面" xmlns="http://www.w3.org/2000/svg" style="font-family:'Microsoft YaHei','Segoe UI',sans-serif"><rect width="640" height="360" fill="${bg}"/><rect width="640" height="5" fill="${accent}" opacity=".7"/><text x="46" y="51" font-size="11" fill="${accent}" letter-spacing="2">课页 / PERSONAL COURSE LIBRARY</text><text x="46" y="137" font-size="32" font-weight="600" fill="${accent}">${esc(line1)}</text><text x="46" y="184" font-size="32" font-weight="600" fill="${accent}">${esc(line2)}</text><rect x="46" y="217" width="38" height="3" fill="${accent}" opacity=".5"/><text x="46" y="261" font-size="14" fill="${accent}">${esc(m.day)}　·　${m.pages} 页</text><path d="M455 217h101v93H455Zm-18 17v94h100" fill="none" stroke="${accent}" stroke-width="4" opacity=".09"/><path d="M45 323h550" stroke="${accent}" opacity=".15"/><text x="46" y="343" font-size="10" fill="${accent}" opacity=".6">${m.source==='import'?'本地导入资料':'华工视频平台课件'} · 自动生成封面</text></svg>`;
}
...
function renderFlatAcquire(){
  ...
  main.innerHTML=`<div class="page-heading"><div><div class="eyebrow">BRING YOUR MATERIALS TOGETHER</div><h1>获取课件</h1><p class="subtitle">从课堂平台获取，或把已有资料带进来。</p></div><span class="badge ${view.loggedIn?'exported':'pending'}">${view.loggedIn?'学校平台已连接':'尚未连接学校平台'}</span></div><div class="source-row"><section class="panel source-card"><div class="source-logo">${icon('school')}</div><div><h3>华工视频平台</h3><p class="${view.loggedIn?'connected':''}">${view.loggedIn?'登录态已导入 · 仅保存在本次运行中':'连接学校平台，按上课日期查找课件'}</p></div><button data-action="login" class="secondary">${view.loggedIn?'重新登录':'连接平台'}</button>${view.loggedIn?'<button class="quiet" data-action="logout">断开</button>':''}</section><section class="panel source-card"><div class="source-logo" style="background:#f2f1ed;color:#979481">${icon('folder')}</div><div><h3>本地资料</h3><p>图片、PDF 或旧版下载目录</p></div><button data-action="import">导入 ${icon('plus')}</button></section></div>
...
async function mutatePages(pages,exclude){
  ...
  finally{view.saving=false;render();}
}
async function undo(){
  if(view.saving||!view.undo.length)return;
  const m=material(),before=view.undo[view.undo.length-1];view.saving=true;
...
function preview(page){
  ...
  call('lastPage',{id:m.id,page:view.currentPage}).catch(fail);
...
function showHelp(){showModal('欢迎使用课页',`<ol class="help-steps"><li>获取课件 → 连接平台，完成学校登录。</li><li>扫描实际有课的日期，选择课程并获取。</li><li>资料库 → 打开课件，筛选需要保留的页面。</li><li>导出 PDF，选择保存位置。</li><li>下次打开课页，继续之前的整理。</li></ol><p>也可以导入本地图片、PDF 或旧版下载目录。封面由课页自动生成，预览展示真实页面。方向键翻页，E 排除/恢复，Ctrl+Z 撤销。</p>`, '<button class="primary" data-action="close-modal">开始使用</button>');}
...
async function handleClick(event){
  ...
  if(action==='download'){const ids=[...view.scanSelected];view.scanSelected.clear();await call('download',{ids,direct:view.directExport});toast('课件已加入后台任务');return;}
  ...
  if(action==='import-folder'||action==='import-files'){closeModal();const added=await call('import',{kind:action==='import-folder'?'folder':'files',courseId:view.importCourseId});if(added){navigate('tasks');toast('正在导入本地资料');}return;}
  ...
  if(action==='toggle-fullscreen-page'){await mutatePages([view.currentPage],!material().excluded.includes(view.currentPage));showFullscreen();return;}
  if(action==='export')return showExport();
  if(action==='quick-export'){await call('quickExport',{id:material().id});toast('已快速导出到默认目录，可在任务页查看进度');return;}
...
document.addEventListener('dblclick',event=>{const el=event.target.closest('[data-preview]');if(el){clearTimeout(preview.timer);view.currentPage=Number(el.dataset.preview);call('lastPage',{id:material().id,page:view.currentPage}).catch(fail);showFullscreen();}});
document.addEventListener('keydown',event=>{
  ...
  if(event.key.toLowerCase()==='e'&&!event.ctrlKey&&!event.metaKey){const full=modal.open;mutatePages([view.currentPage],!material().excluded.includes(view.currentPage)).then(()=>{if(full)showFullscreen();}).catch(fail);}
  if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='z'){event.preventDefault();const full=modal.open;undo().then(()=>{if(full)showFullscreen();}).catch(fail);}
});
...
main.innerHTML=empty('正在打开资料库…','请稍候');hydrate();
if(typeof qt==='undefined')main.innerHTML=empty('请从桌面应用打开课页','在项目目录运行 .venv/Scripts/python.exe app.py');
else new QWebChannel(qt.webChannelTransport,channel=>{
  desktop=channel.objects.desktop;
  desktop.response.connect((id,payload)=>{const p=pending.get(id);if(!p)return;pending.delete(id);const data=JSON.parse(payload);data.ok?p.resolve(data.result):p.reject(new Error(data.error));});
